...
export default function ProductDetailPage() {
  ...
  const [revPagination, setRevPagination] = useState({ current_page: 1, last_page: 1, total: 0 });
  const [revPage, setRevPage]           = useState(1);
  const [seller, setSeller]             = useState(null);
  const [isFav, setIsFav]               = useState(false);
  const [myReview, setMyReview]         = useState(null);
  const [hasPurchased, setHasPurchased] = useState(true); // à connecter avec l'auth réelle
  const [loading, setLoading]           = useState(true);
  const [error, setError]               = useState(null);
  const [activeImage, setActiveImage]   = useState(0);
 
  // Chargement produit + favori
  useEffect(() => {
    setLoading(true);
    setError(null);
    getProduct(id)
  ...
  // Chargement avis paginés
  useEffect(() => {
    getProductReviews(id, { page: revPage, per_page: 5 })
      .then((res) => {
        setReviews(res.data.data ?? []);
        setRevPagination(res.data.pagination ?? {});
        // Détecte l'avis de l'utilisateur connecté
        const userId = Number(localStorage.getItem("user_id"));
        if (userId) {
          const mine = (res.data.data ?? []).find((r) => r.user?.id === userId);
  ...
  const handleReviewSubmitted = (newReview) => {
    ...
    getProductReviews(id, { page: 1, per_page: 5 }).then((res) => {
      setReviews(res.data.data ?? []);
      setRevPagination(res.data.pagination ?? {});
      setRevPage(1);
    });
  ...
  const handleReviewDeleted = (reviewId) => {
    ...
    setRevPagination((p) => ({ ...p, total: Math.max(0, (p.total ?? 1) - 1) }));
  };
    ...
    <div style={{ padding: "24px 32px", maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 40, flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1, minWidth: 280 }}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 16 }}>
            <span style={{ fontSize: 28, fontWeight: 700 }}>{Number(price).toFixed(2)} €</span>
        ...
        <div style={{ marginTop: 40, padding: 20, border: "1px solid #e5e7eb", borderRadius: 12 }}>
          ...
          <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
            ...
            <div>
              ...
              {seller.location && (
                <p style={{ margin: "2px 0 0", fontSize: 13, color: "#6b7280" }}>{seller.location}</p>
              )}
            </div>
            <button onClick={() => navigate(`/sellers/${seller.id}`)}
              style={{
      ...
      <div style={{ marginTop: 40 }}>
        <h2 style={{ fontSize: 18, fontWeight: 600, marginBottom: 20 }}>
          Avis clients ({revPagination.total ?? reviews.length})
        </h2>
 
        {/* Formulaire avis (visible après achat) */}
        {hasPurchased && (
          <ReviewForm
            productId={id}
...
function ReviewCard({ review }) {
  ...
  return (
    <div style={{ padding: 16, border: "1px solid #e5e7eb", borderRadius: 10 }}>
      ...
      {review.comment && (
        <p style={{ margin: "0 0 6px", fontSize: 14, color: "#374151" }}>{review.comment}</p>
      )}
